...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--multiple',
        help="(optional) to reconstruct multiple recordings "
             "[example: ../dataset/].",
        type=str)
    ...
    if not args.multiple:
        ...
        items = os.listdir(args.multiple)

        # Filter out only directories
        folders = [item for item in items if os.path.isdir(os.path.join(args.multiple, item))]
        print(folders)

        for name in folders:
            file_path = 'config/realsense.json'
            with open(file_path, 'r') as file:
                data = json.load(file)
            data['path_dataset'] = f'../dataset/{name}/'
            data["path_intrinsic"] = f"../dataset/{name}/camera_intrinsic.json"
